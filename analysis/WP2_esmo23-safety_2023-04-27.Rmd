...
tbl_soc <- 
  ...
  flextable::save_as_docx(
    path = here("output/PHERGain_teaes_table_2023-05-05.docx")
  )
```
...
data_03b_aes_cohortBprime <- merge(
...
data_04b_aes_cohortBprime_ms <- merge(
  data_05_groupB_idfs_pet_pcr,
  data_04_aes_cohort_ms,
  by = "participant",
  all = FALSE
...
```{r}
# Select treatment emergent-only adverse events
data_03b_aes_cohortBprime <- data_03b_aes_cohortBprime %>%
  subset(
    treatment_emergent_adverse_event_teae == "Yes"
    )
...
# Currently, there is no argument in the tbl_ae() function that provides this functionality
# Filter each adverse event by its maximum severity:
data_04c_aes_cohortBprime_ms <- data_03b_aes_cohortBprime %>% 
  group_by(participant, preferred_term) %>%
  filter(severity == max(severity)) %>%
  distinct(participant, preferred_term, .keep_all = TRUE) %>% 
  group_by(participant) %>% 
...
tbl_soc <- 
  ...
  as_flex_table() %>%
  flextable::save_as_docx(
    path = here("output/PHERGain_teaes_table_groupB'_2023-05-05.docx")
  )
```
